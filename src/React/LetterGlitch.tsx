...
const LetterGlitch = ({
  ...
  const initializeLetters = (columns: number, rows: number) => {
    ...
    const startCol = Math.floor((columns - 5) / 2);

    letters.current = Array.from({ length: totalLetters }, (_, i) => {
      const x = i % columns;
      const y = Math.floor(i / columns);

      let fixedChar: string | null = null;
      if (y === midRow) {
        if (x === startCol) fixedChar = "M";
        if (x === startCol + 2) fixedChar = "N";
        if (x === startCol + 4) fixedChar = "K";
      }

      return {
        char: fixedChar ?? getRandomChar(),
